#!/usr/bin/env python3
"""
Weenie Wars CI Update Script
Runs in GitHub Actions: fetches sheet, updates scores, rebuilds, deploys to Firebase.
"""
import urllib.request, csv, io, hashlib, json, time, os, re, sys, subprocess, random, shutil
from datetime import datetime, timedelta

BUILD_SCRIPT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "build_weenie_wars_widget.py")
HTML_OUT     = os.path.join(os.path.dirname(os.path.abspath(__file__)), "WeeniesWars_2026.html")
...
NICK_UPDATES = [
    ...
    "DNA analysis of a suspicious frankfurter found in Nick's trash revealed it was not a real weenie by any legal definition.",
...
# ── Build HTML ────────────────────────────────────────────────────────────────
result = subprocess.run([sys.executable, BUILD_SCRIPT], capture_output=True, text=True, cwd=REPO_DIR)
if result.returncode != 0:
    print("BUILD ERROR:", result.stderr); sys.exit(1)
print(result.stdout.strip())

# ── Copy to public dir for Firebase ──────────────────────────────────────────
os.makedirs(PUBLIC_DIR, exist_ok=True)
shutil.copy(HTML_OUT, os.path.join(PUBLIC_DIR, "index.html"))

# ── Deploy to Firebase ────────────────────────────────────────────────────────
print("Deploying to Firebase...")
result = subprocess.run(
    ["firebase", "deploy", "--only", "hosting", "--project", "weenie-wars-2026"],
    capture_output=True, text=True, cwd=REPO_DIR
)
if result.returncode == 0:
    print(f"✓ Live: https://weenie-wars-2026.web.app  ({today.strftime('%Y-%m-%d %H:%M')})")
else:
